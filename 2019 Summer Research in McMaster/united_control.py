...
def draw_frames(faces,gray,img,count,face_id,num,supply):
    if supply == str(0):
        for x,y,w,h in faces:
        ...
        for x,y,w,h in faces:
          ...
          #  print "face_gray=",face_gray
            ...
            if(access==False):
                os.mkdir('/home/pi/FRP/dataset/'+str(face_id))
    #        cv2.SaveImage("dataset/User."+str(face_id)+"."+str(count)+".jpg",face_gray)
                cv2.imwrite('/home/pi/FRP/dataset/'+str(face_id)+'/'+str(supply)+".pgm",face_gray)
            elif(access==True):
                cv2.imwrite('/home/pi/FRP/dataset/'+str(face_id)+'/'+str(supply)+".pgm",face_gray)
            cv2.imshow("camera",img)
            count+=1
...
def car_start():
        k=0;stop='n';response4=''
        print '''car start!

I will guide you to your destination,
Please follow me! Thanks ^.^
'''
        print("Initializing: pi send order")
        print ""
        while (1):
            ser.write('k');
#            print("pi send order k")
...
#            print"=============end"
            if (str(response)==str(2)):
                print "Arduino has gotten order! Origional position has been reseted.";
                break    
        while(stop=='n'):
#            print "Jin Ru Xun Huan"
            while (1):
#                ser.write(1)
                ser.write('s');
                print("pi send order")
...
#                print"=============end"
                if (str(response)==str(1)):
                    print "Arduino has gotten order! Please give coordinate of your destination";
                    break    
            if k!=0:
                print "Sorry, please give me new coordinate"
            if stop=='n' or k==0 :
                x=float(raw_input("input x coordinate:  "))
                x=str(x+0.01)
                while(1):
                    ser.write(x+'\n')
...
#                        print "turn to y";
                        break
                y=float(raw_input("input y coordinate:  "))
                y=str(y+0.01)
                while(1):
                    ser.write(y+'\n')
...
#            print "response3:  ",response3
                    if response3!='':
                        print response3
            while(response4=='' or response4!='E'):
                response4=ser.readline()
#            print "response3:  ",response3
                if response4!=''and response4!='E':
                    print response4
            stop=raw_input("Have you arrived your destination? [y/n]")
            k+=1;response4=''
        return stop

def car_back():
        k=0;return1='n';response4=''
        print("Initializing: pi send order")
        print ""
        while (1):
                ser.write('s');
                ...
                response = ser.readline();#read a string from port
                if (str(response)==str(1)):
                    print "Arduino has gotten order! Car goes back to the origional point.";
                    break    
        x=0
        x=str(x+0.01)
        while(1):
        ...
        while(1):
            ...
            response2=ser.readline()  
            if response2=='y':
                print "car starts to go home";break
        response3=str()
        while(response3==''):
            response3=ser.readline()
            if response3!='':
...
def setup():
    ...
    GPIO.setup(sPin, GPIO.IN, pull_up_down=GPIO.PUD_UP)    # Set BtnPin's mode is input, and pull up to high level(3.3V)
            ...
            if (confidence > 80 or error_time>=4) :
                ...
                if (unknown_time%3==0 or error_time>=4):
                    ...
                    if answer2=="y":
                        ...
                        camera.set(cv2.CAP_PROP_FRAME_WIDTH,480)
                        camera.set(cv2.CAP_PROP_FRAME_HEIGHT,320)
                        count=0;id_cards=[];num=0;supply=str(0)
                        ...
                        #           print "read img=",img
                                    ...
                                    count=draw_frames(faces,gray,img,count,face_id,id,supply)
                    ...
                    #           print "read img=",img
                                faces,gray,img=get_faces(img)
                                count=draw_frames(faces,gray,img,count,face_id,num,supply)
                                k=cv2.waitKey(100) & 0xff
                                if(k==27):
                                    break
                                elif(count==100):
